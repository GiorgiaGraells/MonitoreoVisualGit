...
nudge_x=-5000, nudge_y = 1000)+ 
  theme_classic() + scale_fill_viridis_c("Occupancy", option = "plasma", direction = -1) + labs(x = NULL, y = NULL) + facet_wrap(~Species, nrow=2)+ theme(legend.position = "bottom")
...
OcuP <- ggplot() + geom_raster(data = Species_DF, aes(x = x, y = y, fill = Occupancy)) + geom_sf(data = Sitios, color="red") +
    ggrepel::geom_text_repel(data = Sitios,  aes(label = Sitio,  geometry = geometry),  stat = "sf_coordinates", force = 1, 
nudge_x=-5000, nudge_y = 1000)+   theme_classic() + scale_fill_viridis_c("Occupancy", option = "plasma", direction = -1) + 
  labs(x = NULL, y = NULL) + facet_wrap(~Species, nrow=2)+ theme(legend.position = "bottom")
...
AAAABBB
"
OcuI + OcuP+plot_layout(design = layout, guides = 'collect') +plot_annotation(tag_levels = 'A') 
