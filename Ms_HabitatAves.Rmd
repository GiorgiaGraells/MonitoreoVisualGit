...
Sitios <- read.csv("/home/giorgia/Documents/Doctorado tesis/Monitoreo aves/MonitoreoVisualGit/Muestreo aves jun-jul 2019/Monitoreo_punto/Reg_completo_aves_inv.csv") %>% 
  dplyr::filter(Sitio %in% c("PLAZA ANIBAL PINTO", "PLAYA LA BOCA", "PLAZA GABRIELA MISTRAL", "LAGUNA VERDE")) %>% 
  mutate(Sitio= str_replace(Sitio, "PLAZA ANIBAL PINTO", "PAP"),
         Sitio= str_replace(Sitio, "PLAYA LA BOCA", "PLB"),
         Sitio= str_replace(Sitio, "PLAZA GABRIELA MISTRAL", "PGM"),
         Sitio= str_replace(Sitio, "LAGUNA VERDE", "LG")) %>% 
...
ggplot() + geom_raster(data = Species_DF, aes(x = x, y = y, fill = Occupancy)) + geom_sf(data = Sitios, color="red") +
...
nudge_x=-5000, nudge_y = 1000)+ 
  theme_classic() + scale_fill_viridis_c("Occupancy", option = "plasma", direction = -1) + labs(x = NULL, y = NULL) + facet_wrap(~Species, nrow=2)
...
names(Especies) <- c("Columba livia", "Larosterna inca", "Larus dominicanus", "Leucophaeus pipixcan", "Pelecanus thagus", "Phalacrocorax bougainvillii")
